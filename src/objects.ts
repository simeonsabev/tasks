...
export function isValid(question: Question, answer: string): boolean {
    ...
    if (question.type === "short_answer_question") {
        ...
        boolean = question.options.includes(answer);
    }
    return boolean;
}
...
export function toShortForm(question: Question): string {
    ...
    const nameShortened = question.name.substring(0, 10);
    return `${idString}: ${nameShortened}`;
}
...
export function toMarkdown(question: Question): string {
    ...
    if (question.type === "multiple_choice_question" && question.options) {
        result += "\n";
        result += question.options.map((option) => `- ${option}`).join("\n");
    }
